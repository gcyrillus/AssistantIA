...
def mettre_a_jour_modeles(api_key):
    """Va chercher le catalogue massif de modèles d'OpenRouter"""
    url = "https://openrouter.ai/api/v1/models"
    req = urllib.request.Request(url)
    req.add_header("Authorization", f"Bearer {api_key}")
    req.add_header("Content-Type", "application/json")
    # Headers spécifiques recommandés par OpenRouter
    req.add_header("HTTP-Referer", "https://github.com/ton-repo/AssistantIA") 
    req.add_header("X-Title", "AssistantIA Notepad++")
    ...
    try:
        with urllib.request.urlopen(req, timeout=10, context=ctx) as response:
            ...
            # OpenRouter renvoie énormément de modèles. On les prend tous.
            for m in donnees.get("data", []):
                m_id = m.get("id")
                # Vérifiez si le modèle est gratuit
                if m.get("price", 0) == 0 or m.get("is_free", False):
        ...
        print(msg.format(provider="{{API_LOCAL_NAME}}", e=str(e)))
        return {"error": str(e)}
...
def enregistrer_historique_dans_fichier(provider, modele):
    ...
    try:
        ...
        print(msg.format(provider={{PROVIDER_NAME}}, e=str(e)))
        return None
...
def envoyer_chat(prompt, stocker_log, modele_a_la_volee=None):
    ...
    if not modele:
        # Modèle gratuit et rapide par défaut chez OpenRouter
        modele = "meta-llama/llama-3-8b-instruct:free"
    ...
    # URL de complétion standard OpenRouter
    ...
    try:
        ...
        return _T("MESSAGES", "MSG_PROV_ERR_API").format(provider="Groq", e=str(e))
...
def markdown_to_rtf(text_md):
    ...
    for line in lines:
        ...
        if dans_code:
            ...
            if escaped_line.startswith("### "): escaped_line = re.sub(r'^### (.*)', r'\\pard\\b\\fs24 \1\\b0\\fs20\\par', escaped_line)
            elif escaped_line.startswith("## "): escaped_line = re.sub(r'^## (.*)', r'\\pard\\b\\fs28 \1\\b0\\fs20\\par', escaped_line)
            elif escaped_line.startswith("# "): escaped_line = re.sub(r'^# (.*)', r'\\pard\\b\\fs32 \1\\b0\\fs20\\par', escaped_line)
            elif re.match(r'^[\*\-] ', escaped_line): escaped_line = re.sub(r'^[\*\-] (.*)', r'\\pard\\li300\\bullet  \1\\par', escaped_line)
            elif re.match(r'^\d+\. ', escaped_line): escaped_line = re.sub(r'^(\d+\.) (.*)', r'\\pard\\li300 \1 \2\\par', escaped_line)
            else: escaped_line = r"\pard\li0 " + escaped_line + r"\par"
...
def demander_code_sur_selection(consigne, code_selectionne, sauvegarder_log=True):
    ...
    if not modele: modele = "meta-llama/llama-3-8b-instruct:free"
